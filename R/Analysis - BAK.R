...
plot(xts(100*apply(stocks.HFTpquotes.df, 1, mean, na.rm=T, trim=.001), 
         as.Date(rownames(stocks.HFTpquotes.df)))['2006-01-03/2013-08-28'])
...
hist(na.remove(log(stocks.idret.tail.df+0.0001)))
...
plot(xts(rowSums(stocks.NMS.df[range.idx,], na.rm=T), 
         as.Date(rownames(stocks.HFTflags.df[range.idx,]))), 
     main="Introduction of RegNMS")

plot(xts(rowSums(stocks.HFTflags.df[range.idx,], na.rm=T), 
         as.Date(rownames(stocks.HFTflags.df[range.idx,]))), 
     main="Introduction of RegNMS")


# plot(xts(rowMeans(stocks.idret.tail.df[range.idx,], na.rm=T), 
#          as.Date(rownames(stocks.HFTflags.df[range.idx,]))), 
#      main="Introduction of RegNMS")
...
plot(xts(rowSums(stocks.SSB.df[range.idx,], na.rm=T), 
         as.Date(rownames(stocks.HFTflags.df[range.idx,]))), 
     main="Extent of the SEC short-sale ban")

plot(xts(rowMeans(StdDf(stocks.score.min[range.idx,]), na.rm=T), 
         as.Date(rownames(stocks.HFTflags.df[range.idx,]))), 
     main="Extent of the SEC short-sale ban")

plot(xts(rowMeans(log(stocks.idret.tail.df[range.idx,]+0.0001), na.rm=T), 
         as.Date(rownames(stocks.HFTflags.df[range.idx,]))), 
     main="Extent of the SEC short-sale ban")
...
GetCompleteStocks <- function() {
  ...
  stocks.completeE1 <- apply (get("stocks.score.max")[range.idx,], 2, 
                              function(x) any(is.na(x) | x > 100))
  summary(stocks.completeE1)
  stocks.completeE2 <- apply (get("stocks.score.min")[range.idx,], 2, 
                              function(x) any(is.na(x) | x < -100))
...
# select time-matched EventDB
events.sel.df <- events.master.up.df[events.master.up.df$symbol %in%
                   names(stocks.HFTflags.df)[!stocksel.complete] &
                   as.Date(events.master.up.df$timestamp) %in% 
                   as.Date(rownames(stocks.HFTflags.df[range.idx,])), ]

events.sel.df$NMS

sapply(1:nrow(events.sel.df), function(x) 
  stocks.NMS.df[as.character(as.Date(events.master.up.df$timestamp))[x],
                events.sel.df$symbol[x])

stocks.NMS.df[as.character(as.Date(events.master.up.df$timestamp)),
                                   events.sel.df$symbol]

# --> stocks.complete & range.idx to be used for panel construction function
...
# check bivariate distributions to identify outlier/erroneous data
...
# check VIFs
with(stocks.panel.pdf[stocks.panel.pdf$symbol %in% stock.sel,],{
  Vif(cbind(log1p(HFL), VOLE, log(TOV), SIZE, PR, BETA), thresh=10, trace=T)
})
with(stocks.panel.pdf[stocks.panel.pdf$symbol %in% stock.sel,],{
...
# 1st step: HFL
check.res <- felm(log((HFL+1))~NMS+(VOLE)+SIZE+PR+log(TOV)+G(date)+G(symbol), 
...
summary(check.res)
# 1st step: HPF
check.res <- felm((log(HPF+0.01))~NMS+VOLE+SIZE+PR+log(TOV)+G(date)+G(symbol), stocks.panel.pdf, clustervar="date")
qqPlot(check.res$residuals, main="HPF")
HPF.hat <- check.res$fitted
# 1st step: HPQ
check.res <- felm((log(HPQ+0.01))~NMS+VOLE+SIZE+PR+log(TOV)+G(date)+G(symbol), stocks.panel.pdf, clustervar="date")
qqPlot(check.res$residuals, main="HPQ")
HPQ.hat <- check.res$fitted
# 1st step: HPFL
check.res <- felm((log(HPFL+0.0001))~NMS+VOLE+SIZE+PR+log(TOV)+G(date)+G(symbol), stocks.panel.pdf, clustervar="date")
qqPlot(check.res$residuals, main="HPFL")
HPFL.hat <- check.res$fitted
# 1st step: HPQL
check.res <- felm((log(HPQL+0.0001))~NMS+VOLE+SIZE+PR+log(TOV)+G(date)+G(symbol), stocks.panel.pdf, clustervar="date")
qqPlot(check.res$residuals, main="HPQL")
HPQL.hat <- check.res$fitted

# 2nd step: HFL
check.res <- felm(sqrt(EEU)~HFL.hat+(VOLE)+SIZE+PR+log(TOV)+G(date)+G(symbol), 
                  stocks.panel.pdf, clustervar=interaction(stocks.panel.pdf$symbol, 
                                                           stocks.panel.pdf$date))
qqPlot(check.res$residuals, main="EEU ~ HFL")
summary(check.res)
check.res <- felm(sqrt(EED)~HFL.hat+(VOLE)+SIZE+PR+log(TOV)+G(date)+G(symbol), 
                  stocks.panel.pdf, clustervar=interaction(stocks.panel.pdf$symbol, 
                                                           stocks.panel.pdf$date))
qqPlot(check.res$residuals, main="EED ~ HFL")
summary(check.res)
check.res <- felm(sqrt(EEU+EED)~HFL.hat+(VOLE)+SIZE+PR+log(TOV)+G(date)+G(symbol), 
                  stocks.panel.pdf, clustervar=interaction(stocks.panel.pdf$symbol, 
                                                           stocks.panel.pdf$date))
...
# 2nd step: HPFL
check.res <- felm(sqrt(EEU)~HPFL.hat+(VOLE)+SIZE+PR+log(TOV)+G(date)+G(symbol), 
                  stocks.panel.pdf, clustervar=interaction(stocks.panel.pdf$symbol, 
                                                           stocks.panel.pdf$date))
qqPlot(check.res$residuals, main="EEU ~ HPFL")
summary(check.res)
check.res <- felm(sqrt(EED)~HPFL.hat+(VOLE)+SIZE+PR+log(TOV)+G(date)+G(symbol), 
                  stocks.panel.pdf, clustervar=interaction(stocks.panel.pdf$symbol, 
                                                           stocks.panel.pdf$date))
qqPlot(check.res$residuals, main="EED ~ HPFL")
summary(check.res)
check.res <- felm(sqrt(EEU+EED)~HPFL.hat+(VOLE)+SIZE+PR+log(TOV)+G(date)+G(symbol), 
                  stocks.panel.pdf, clustervar=interaction(stocks.panel.pdf$symbol, 
                                                           stocks.panel.pdf$date))
qqPlot(check.res$residuals, main="EEU+EED ~ HPFL")
summary(check.res)
...
HFL.hat <- check.res$fitted
summary(check.res)
check.res <- felm(IRS~HFL.hat+(VOLE)+SIZE+PR+(TOV)+G(date)+G(symbol), 
                  stocks.panel.pdf, clustervar=interaction(stocks.panel.pdf$symbol, 
                                                           stocks.panel.pdf$date))
qqPlot(check.res$residuals, main="VOLE ~ HFL")
summary(check.res)

names(stocks.panel.pdf)

# 3c) Winsorization & Distribution Check ---------------------------------------


# Winsorization
(names(stocks.panel.pdf))
for (i in c(3:4, 7:ncol(stocks.panel.pdf))) {
...
hist(stocks.panel.pdf$EEU)
hist(stocks.panel.pdf$EED)

# 4) Panel Analysis ------------------------------------------------------------
# construct size / liquidity quintile factors based on stock-averages in sample
...
panel.cl <- factor(rank.cl[stocks.panel.pdf$symbol,], 
                   levels=c("L", "M", "H"), ordered=T)
length(panel.cl)

# 4a) Check instrument ---------------------------------------------------------
Reg1stStage(stocks.panel.pdf, formula(HFL~NMS+G(date)+G(symbol)), panel.cl, T)
Reg1stStage(stocks.panel.pdf, formula(log1p(HFL)~NMS+G(date)+G(symbol)), panel.cl)
...
with(stocks.panel.pdf, hist(HFL))
with(stocks.panel.pdf, hist(log1p(HFL)))
with(stocks.panel.pdf, hist(HPF))
with(stocks.panel.pdf, summary(HPF))
with(stocks.panel.pdf, hist(log(HPF+.1)))
with(stocks.panel.pdf, hist(HPQ))
with(stocks.panel.pdf, hist(log(HPQ+.1)))
with(stocks.panel.pdf, hist(HPFL))
with(stocks.panel.pdf, summary(HPFL))
with(stocks.panel.pdf, hist(log(HPFL+.01)))
with(stocks.panel.pdf, hist(HPQL))
with(stocks.panel.pdf, summary(HPQL))
with(stocks.panel.pdf, hist(log(HPQL+.01)))

with(stocks.panel.pdf, scatterplot(y=EEU+EED, x=VOLA))
with(stocks.panel.pdf, scatterplot(y=EEU+EED, x=VOLE))
with(stocks.panel.pdf, scatterplot(y=EEU+EED, x=sqrt(VOLE)))
with(stocks.panel.pdf, scatterplot(y=EEU+EED, x=log(VOLE)))
with(stocks.panel.pdf, scatterplot(y=EEU+EED, x=VOL))
with(stocks.panel.pdf, scatterplot(y=EEU+EED, x=LIQ))
with(stocks.panel.pdf, scatterplot(y=EEU+EED, x=TOV))
with(stocks.panel.pdf, scatterplot(y=EEU+EED, x=sqrt(TOV)))
...
Reg1stStage(stocks.panel.pdf, formula((EEU)~NMS+G(date)+G(symbol)), panel.cl, T, F)
Reg1stStage(stocks.panel.pdf, formula((EED)~NMS+G(date)+G(symbol)), panel.cl, T, F)
Reg1stStage(stocks.panel.pdf, formula((EEU+EED)~NMS+G(date)+G(symbol)), panel.cl, T, F)
...
# HFL 1st stage fit
reg.form <- formula (log1p(HFL)~NMS+VOLE+PR+SIZE+TOV+G(date)+G(symbol))
HFL.res  <- felm (reg.form, stocks.panel.pdf, 
                 clustervar=interaction(stocks.panel.pdf$symbol, 
                                        stocks.panel.pdf$date))
summary(HFL.res)
HFL.hat <- HFL.res$fitted
# HPF 1st stage fit
reg.form <- formula (log(HPF+.1)~NMS+VOLE+PR+SIZE+TOV+G(date)+G(symbol))
HPF.res  <- felm (reg.form, stocks.panel.pdf, 
                  clustervar=interaction(stocks.panel.pdf$symbol, 
                                         stocks.panel.pdf$date))
summary(HPF.res)
...
# HPQ 1st stage fit
reg.form <- formula (log(HPQ+0.1)~NMS+VOLE+PR+SIZE+TOV+G(date)+G(symbol))
HPQ.res  <- felm (reg.form, stocks.panel.pdf, 
                  clustervar=interaction(stocks.panel.pdf$symbol, 
                                         stocks.panel.pdf$date))
summary(HPQ.res)
HPQ.hat <- HPQ.res$fitted

# HPFL 1st stage fit
reg.form <- formula (log(HPFL+.1)~NMS+VOLE+PR+SIZE+TOV+G(date)+G(symbol))
HPFL.res  <- felm (reg.form, stocks.panel.pdf, 
                  clustervar=interaction(stocks.panel.pdf$symbol, 
                                         stocks.panel.pdf$date))
summary(HPFL.res)
HPFL.hat <- HPFL.res$fitted

# HPQL 1st stage fit
reg.form <- formula (log(HPQL+0.1)~NMS+VOLE+PR+SIZE+TOV+G(date)+G(symbol))
HPQL.res  <- felm (reg.form, stocks.panel.pdf, 
                  clustervar=interaction(stocks.panel.pdf$symbol, 
                                         stocks.panel.pdf$date))
summary(HPQL.res)
HPQL.hat <- HPQ.res$fitted

# HFTDx 1st stage fit
reg.form <- formula (log1p(HPFD5)~NMS+VOLE+log1p(VOLD5)+PR+SIZE+TOV+G(date)+G(symbol))
HFTD.res  <- felm (reg.form, stocks.panel.pdf, 
                  clustervar=interaction(stocks.panel.pdf$symbol, 
                                         stocks.panel.pdf$date))
summary(HFTD.res)
HFTD.hat <- HFTD.res$fitted

# HFTUx 1st stage fit
reg.form <- formula (log1p(HPFU5)~NMS+VOLE+log1p(VOLU5)+PR+SIZE+TOV+G(date)+G(symbol))
HFTU.res  <- felm (reg.form, stocks.panel.pdf, 
                    clustervar=interaction(stocks.panel.pdf$symbol, 
                                           stocks.panel.pdf$date))
summary(HFTU.res)
HFTU.hat <- HFTU.res$fitted

# manual 2nd stage regression
hist(stocks.panel.pdf$IRS)
hist(log(stocks.panel.pdf$IRS))

HFL.res2a <- felm (IET~HPQL.hat+VOLE+PR+SIZE+TOV+G(date)+G(symbol), 
                   stocks.panel.pdf,
                   clustervar=interaction(stocks.panel.pdf$symbol, 
                                          stocks.panel.pdf$date))
summary(HFL.res2a)

HFL.res2a <- felm (EED~HFTD.hat+log1p(VOLD5)+VOLE+PR+SIZE+TOV+G(date)+G(symbol), 
                   stocks.panel.pdf,
                   clustervar=interaction(stocks.panel.pdf$symbol, 
                                          stocks.panel.pdf$date))
summary(HFL.res2a)

HFL.res2a <- felm (EEU~HFTU.hat+log1p(VOLU5)+VOLE+PR+SIZE+TOV+G(date)+G(symbol), 
                   stocks.panel.pdf,
                   clustervar=interaction(stocks.panel.pdf$symbol, 
                                          stocks.panel.pdf$date))
summary(HFL.res2a)
...
HPQ.res2 <- felm (IET~IEM+HPQ+VOLE+PR+SIZE+TOV+G(date)+G(symbol), 
...
# HFL IVREG
HFL.res2 <- felm ((EED+EEU)~HFL+VOLE+PR+SIZE+TOV+G(date)+G(symbol), 
                  stocks.panel.sub, iv=HFL~NMS, 
